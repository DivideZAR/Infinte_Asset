...
async function getFrameSize(frameDir: string): Promise<{ width: number; height: number } | null> {
  ...
  try {
    const framePath = path.join(frameDir, firstFrame)
...
async function generateThumbnail(
  ...
  const targetFrame = frameNumber < frames.length ? frames[frameNumber] : frames[frames.length - 1]
